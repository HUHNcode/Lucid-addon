...
public class AutoTotem extends Module
{
    ...
    public AutoTotem()
    {
        super(LucidAddon.CATEGORY, "auto-pop-it", "Best auto totem for 1.17+");
        instance = this;
        // Validate.notNull(Offhand.instance); // Replaced with dynamic fetching and null checks
    }
    ...
    private void tick(TickEvent.Pre event)
    {
        ...
        Offhand offhandModule = Modules.get().get(Offhand.class);

        if (offhandModule != null && offhandModule.isActive() && SmartCheck(offhandModule))
        {
    ...
    private void onPacketReceived(PacketEvent.Receive event)
    {
        ...
        {
            if (packet.getStatus() != 35 || packet.getEntityId() != mc.player.getId()) return;
    ...
    // Crystal damage calculation: (float)((int)((1D + 1D) / 2.0D * 7.0D * 12.0D + 1.0D)) = 85.0
    private static final double CRYSTAL_DAMAGE = 85.0;
    private boolean SmartCheck(Offhand offhandModule)    // TODO: check wither explosion damage too
    {
        if (offhandModule == null) return false; // Cannot perform smart check without Offhand module

        if (mc.player.isGliding()) return false; // In 1.17+ isGliding() is used, or check mc.player.getPose() == EntityPose.FALL_FLYING
        if (GetLatency() >= 125) return false;  // TODO: assume TPS: 2.5 * interval_per_tick instead of 125

        float health = GetHealth();

        if (offhandModule.cfg_mode.get() == Offhand.OffhandMode.normal)
            return health >= offhandModule.cfg_min_health.get();
        ...
        // Reduce by enchants
        // For 1.17+, getProtectionAmount typically takes (LivingEntity, DamageSource)
        int protLevel = EnchantmentHelper.getProtectionAmount(mc.player, mc.world.getDamageSources().explosion(null, null));
